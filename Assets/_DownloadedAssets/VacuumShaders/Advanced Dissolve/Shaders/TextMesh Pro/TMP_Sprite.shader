...
#include "../cginc/AdvancedDissolve.cginc"
			...
			v2f vert(appdata_full IN)
			{
				...
				//VacuumShaders 
#ifdef _DISSOLVEMAPPINGTYPE_TRIPLANAR
				OUT.coords = float4(IN.vertex.xyz, 1);
				OUT.objNormal = lerp(UnityObjectToWorldNormal(IN.normal), IN.normal, VALUE_TRIPLANARMAPPINGSPACE);
#else
				DissolveVertex2Fragment(OUT.vertex, IN.texcoord, IN.texcoord1, OUT.dissolveUV);
#endif
			...
			fixed4 frag(v2f IN) : SV_Target
			{
...
#ifdef _DISSOLVEMAPPINGTYPE_TRIPLANAR
				float4 alpha = ReadDissolveAlpha_Triplanar(IN.coords, IN.objNormal, IN.worldPosition);
#else
				float4 alpha = ReadDissolveAlpha(IN.texcoord.xy, IN.dissolveUV, IN.worldPosition);
#endif				
				...
				float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.texcoord.xy);
