...
#include "../cginc/AdvancedDissolve.cginc"
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			//VacuumShaders 
			float3 advancedDissolve_worldPos = mul(unity_ObjectToWorld, float4(input.position.xyz, 1)).xyz;
			float3 advancedDissolve_coords = 0;
			float3 advancedDissolve_objNormal = 0;
			float4 advancedDissolve_UV = 0;
#ifdef _DISSOLVEMAPPINGTYPE_TRIPLANAR
			advancedDissolve_coords = float4(input.position.xyz, 1);
			advancedDissolve_objNormal = lerp(UnityObjectToWorldNormal(input.normal), input.normal, VALUE_TRIPLANARMAPPINGSPACE);
#else
			DissolveVertex2Fragment(vPosition, input.texcoord0, input.texcoord1, advancedDissolve_UV);
#endif
...
#else
			float4 alpha = ReadDissolveAlpha(input.atlas.xy, input.dissolveUV, input.worldPos);
#endif				
		...
		float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, input.atlas.xy);
