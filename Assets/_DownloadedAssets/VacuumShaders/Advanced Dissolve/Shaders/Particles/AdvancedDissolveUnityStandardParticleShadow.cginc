    ...
    out float4 opos : SV_POSITION)
{
	...
	o.worldPos = mul(unity_ObjectToWorld, float4(v.vertex.xyz, 1)).xyz;
		...
		#endif
		DissolveVertex2Fragment(oPos, v.texcoords, v.texcoords1, o.dissolveUV);
	#endif
#endif
	...
	#ifdef _DISSOLVEMAPPINGTYPE_TRIPLANAR
		os.coords = float4(v.vertex.xyz, 1);
		os.objNormal = lerp(UnityObjectToWorldNormal(v.normal), v.normal, VALUE_TRIPLANARMAPPINGSPACE);
	#else
		float4 oPos = 0;
		#ifdef _DISSOLVEMAPPINGTYPE_SCREEN_SPACE
		   oPos = UnityObjectToClipPos(v.vertex);
		#endif
		DissolveVertex2Fragment(oPos, v.texcoords, v.texcoords1, os.dissolveUV);
	#endif
#endif
}
...
#ifdef UNITY_STANDARD_USE_SHADOW_OUTPUT_STRUCT
	#ifdef _DISSOLVEMAPPINGTYPE_TRIPLANAR
		float dissolveAlpha = ReadDissolveAlpha_Triplanar(i.coords, i.objNormal, i.worldPos);
	#else
		float dissolveAlpha = ReadDissolveAlpha(i.texcoord.xy, i.dissolveUV, i.worldPos);
	#endif				
