...
#else
float4 alpha = ReadDissolveAlpha(IN.custompack1.xy, IN.dissolveUV, worldPos);
#endif
DoDissolveClip(alpha);

float3 dissolveAlbedo = 0;
float3 dissolveEmission = 0;
float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.custompack1.xy);
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  UNITY_SETUP_INSTANCE_ID(IN);
                    ...
                    #else
					    float4 alpha = ReadDissolveAlpha(IN.custompack1.xy, IN.dissolveUV, IN.worldPos);
                    #endif          
				    DoDissolveClip(alpha);
