...
#else
		  DissolveVertex2Fragment(o.pos, IN.uv0, IN.uv1, o.dissolveUV);
#endif
		  return o;
		}
...
#else
			float4 alpha = ReadDissolveAlpha(IN.uv0.xy, IN.dissolveUV, IN.worldPos);
#endif
			DoDissolveClip(alpha);

			float3 dissolveAlbedo = 0;
			float3 dissolveEmission = 0;
			float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.uv0.xy);
...
#include "../cginc/Integration_CurvedWorld.cginc"
		...
		v2f vert (appdata IN) {
		  ...
		  o.objNormal = lerp(UnityObjectToWorldNormal(IN.normal), IN.normal, VALUE_TRIPLANARMAPPINGSPACE);
#else
		  DissolveVertex2Fragment(o.pos, IN.uv0.xy, IN.uv1.xy, o.dissolveUV);
#endif
		  return o;
		}
		...
		fixed4 frag (v2f IN) : SV_Target 
		{
			...
			float4 alpha = ReadDissolveAlpha_Triplanar(IN.coords, IN.objNormal, IN.worldPos);
#else
			float4 alpha = ReadDissolveAlpha(IN.uv1.xy, IN.dissolveUV, IN.worldPos);
#endif
		...
		float3 dissolveAlbedo = 0;
		float3 dissolveEmission = 0;
		float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.uv1.xy);
...
#include "../cginc/Integration_CurvedWorld.cginc"
		...
		fixed4 frag (v2f IN) : SV_Target 
		{
			...
			float4 alpha = ReadDissolveAlpha_Triplanar(IN.coords, IN.objNormal, IN.worldPos);
#else
			float4 alpha = ReadDissolveAlpha(IN.uv2.xy, IN.dissolveUV, IN.worldPos);
#endif
			DoDissolveClip(alpha);

			float3 dissolveAlbedo = 0;
float3 dissolveEmission = 0;
			float dissolveBlend = DoDissolveAlbedoEmission(alpha, dissolveAlbedo, dissolveEmission, IN.uv2.xy);
