...
inline float4 ReadDissolveAlpha(float2 mainTexUV, float4 dissolveMapUV, float3 vertexPos)
{
	float4 alphaSource = 1;
		...
		screenUV *= distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz);
	#endif
			...
			alphaSource = tex2D(VALUE_MAP1, screenUV);
		#else
			alphaSource = tex2D(VALUE_MAP1, dissolveMapUV.xy);
		#endif
			...
			float4 t1 = tex2D(VALUE_MAP1, uv1);
			float4 t2 = tex2D(VALUE_MAP2, uv2);
		#else
			float4 t1 = tex2D(VALUE_MAP1, dissolveMapUV.xy);
			float4 t2 = tex2D(VALUE_MAP2, dissolveMapUV.zw);
		#endif
		...
		float4 t1 = tex2D(VALUE_MAP1, uv1);
		float4 t2 = tex2D(VALUE_MAP2, uv2);
		float4 t3 = tex2D(VALUE_MAP3, uv3);
	...
	{	
		...
		float4 alphaSource = 1;
			...
			alphaSource = ReadTriplanarTexture(VALUE_MAP1, coords * VALUE_MAP1_ST.x * 0.1 + VALUE_MAP1_SCROLL.xyz * TIME, blend);

		#elif defined(_DISSOLVEALPHASOURCE_TWO_CUSTOM_MAPS)

			float4 t1 = ReadTriplanarTexture(VALUE_MAP1, coords * VALUE_MAP1_ST.x * 0.1 + VALUE_MAP1_SCROLL.xyz * TIME, blend);
			float4 t2 = ReadTriplanarTexture(VALUE_MAP2, coords * VALUE_MAP2_ST.x * 0.1 + VALUE_MAP2_SCROLL.xyz * TIME, blend);
			...
			float4 t1 = ReadTriplanarTexture(VALUE_MAP1, coords * VALUE_MAP1_ST.x * 0.1 + VALUE_MAP1_SCROLL.xyz * TIME, blend);
			float4 t2 = ReadTriplanarTexture(VALUE_MAP2, coords * VALUE_MAP2_ST.x * 0.1 + VALUE_MAP2_SCROLL.xyz * TIME, blend);
			float4 t3 = ReadTriplanarTexture(VALUE_MAP3, coords * VALUE_MAP3_ST.x * 0.1 + VALUE_MAP3_SCROLL.xyz * TIME, blend);
			...
			alphaSource = ReadTriplanarTexture(_MainTex, coords * VALUE_MAIN_MAP_TILING * 0.1, blend).a;
